...
def waitlist_allocation():
    """Bulk waitlist allocation management"""
    from .models import Waitlist
    from datetime import date, timedelta
    
    # Get trains departing tomorrow (candidates for automatic allocation)
    tomorrow = date.today() + timedelta(days=1)
    
    trains_tomorrow = db.session.query(
        Train.id, Train.number, Train.name,
        db.func.count(Booking.id).label('waitlist_count'),
    ...
    ).group_by(Train.id, Train.number, Train.name, Train.available_seats).all()
    
    # Get recent allocation history
    recent_allocations = db.session.query(
        Booking.train_id, Train.number, Train.name,
        db.func.count(Booking.id).label('confirmed_count'),
        db.func.max(Booking.booking_date).label('last_confirmation')
    ).select_from(Booking).join(Train).filter(
        Booking.status == 'confirmed',
        db.func.date(Booking.booking_date) >= date.today() - timedelta(days=7)
    ).group_by(Booking.train_id, Train.number, Train.name).limit(10).all()
...
@admin_bp.route('/waitlist-allocation/allocate-train/<int:train_id>/<journey_date>', methods=['POST'])
@admin_required
def allocate_train_waitlist(train_id, journey_date):
    """Allocate all available seats from waitlist for a specific train and date"""
    from .models import Waitlist
    ...
    # Get waitlisted bookings for this train and date, ordered by position
    waitlisted_bookings = db.session.query(Booking).join(Waitlist).filter(
        Booking.train_id == train_id,
        Booking.journey_date == journey_date_obj,
        Booking.status == 'waitlisted'
    ).order_by(Waitlist.position).all()
    
    if not waitlisted_bookings:
        flash('No waitlisted bookings found for this train and date', 'info')
        return redirect(url_for('admin.waitlist_allocation'))
    
    # Use the waitlist manager to process allocations
    waitlist_manager = WaitlistManager()
    confirmed_bookings = waitlist_manager.process_waitlist(train_id, journey_date_obj)
    
    if confirmed_bookings:
        flash(f'Successfully allocated seats to {len(confirmed_bookings)} bookings from waitlist', 'success')
    else:
        flash('No seats could be allocated from waitlist (no available seats)', 'warning')
    
    return redirect(url_for('admin.waitlist_allocation'))

@admin_bp.route('/waitlist-allocation/allocate-all-tomorrow', methods=['POST'])
@admin_required
def allocate_all_tomorrow():
    ...
    # Get all trains with waitlisted bookings for tomorrow
    trains_with_waitlist = db.session.query(Train.id).join(Booking).filter(
        Booking.status == 'waitlisted',
        Booking.journey_date == tomorrow
    ).distinct().all()
    
    if not trains_with_waitlist:
        flash('No trains with waitlisted bookings found for tomorrow', 'info')
        return redirect(url_for('admin.waitlist_allocation'))
    
    waitlist_manager = WaitlistManager()
    total_confirmed = 0
    
    for (train_id,) in trains_with_waitlist:
        confirmed_bookings = waitlist_manager.process_waitlist(train_id, tomorrow)
        if confirmed_bookings:
            total_confirmed += len(confirmed_bookings)
    
    if total_confirmed > 0:
        flash(f'Successfully allocated seats to {total_confirmed} bookings across {len(trains_with_waitlist)} trains', 'success')
    else:
        flash('No seats could be allocated from any waitlists', 'warning')
...
def schedule_auto_allocation():
    ...
    allocation_time = request.form.get('allocation_time', '18:00')  # Default 6 PM
    
    # Store the setting in database or session (for demo purposes, using session)
