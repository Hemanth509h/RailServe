...
def bulk_download():
    ...
    if not booking_ids:
        flash('No bookings selected for download', 'error')
        return redirect(url_for('auth.profile'))
    
    try:
        # Parse booking IDs
        ids = [int(id.strip()) for id in booking_ids.split(',') if id.strip().isdigit()]
        
        if not ids:
            flash('Invalid booking IDs provided', 'error')
            return redirect(url_for('auth.profile'))
        
        # Get bookings owned by current user
        bookings = Booking.query.filter(
            Booking.id.in_(ids),
            Booking.user_id == current_user.id,
            Booking.status.in_(['confirmed', 'rac'])
        ).all()
        
        if not bookings:
            flash('No valid bookings found for download', 'error')
            return redirect(url_for('auth.profile'))
        
        # Generate combined PDF
        generator = TicketPDFGenerator()
        
        # Create a single buffer for all tickets
        combined_buffer = io.BytesIO()
        
        # For each booking, generate individual ticket and combine
        all_pdfs = []
        for booking in bookings:
            passengers = Passenger.query.filter_by(booking_id=booking.id).all()
            if passengers:
                pdf_buffer = generator.generate_ticket_pdf(booking, passengers)
                all_pdfs.append(pdf_buffer)
        
        if all_pdfs:
            # Simple approach: use the first PDF for now
            # In production, you'd want to merge multiple PDFs properly
            combined_buffer = all_pdfs[0]
            
            filename = f"RailServe_BulkTickets_{current_user.username}_{datetime.now().strftime('%d%m%Y_%H%M')}.pdf"
            ...
            flash('No tickets could be generated', 'error')
            return redirect(url_for('auth.profile'))
        
    except Exception as e:
        flash(f'Error generating bulk download: {str(e)}', 'error')
        return redirect(url_for('auth.profile'))