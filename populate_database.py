...
Populates the database with 1000 trains, 750 stations and comprehensive route network
Run this script from the root directory: python populate_database.py
"""
...
from src.models import User, Train, Station, TrainRoute, Booking, Payment
...
def main():
    ...
    with app.app_context():
        ...
        # Create train routes in smaller batches
        print("Creating train routes...")
        # Process trains in batches to avoid timeout
        batch_size = 200
        for i in range(0, len(trains), batch_size):
            batch_trains = trains[i:i+batch_size]
            print(f"Processing routes for trains {i+1} to {min(i+batch_size, len(trains))}")
            populate_train_routes(batch_trains, stations[:100])  # Use first 100 stations for routes
